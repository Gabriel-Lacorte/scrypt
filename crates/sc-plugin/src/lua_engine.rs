...
impl LuaDissector {
    fn new(path: &Path) -> Result<Self> {
        ...
        let field_fn = lua
            .create_function(
                |lua_ctx, (name, display_value, offset, length): (String, String, usize, usize)| {
...
impl Dissector for LuaDissector {
    ...
    fn dissect<'a>(
        ...
        let func: LuaFunction = globals.get("dissect").map_err(|e| ShadowError::Plugin {
            message: format!(
                "Lua script {} missing 'dissect' function: {e}",
                self.script_path
            ),
        })?;
        ...
        if let Ok(fields_table) = result.get::<LuaTable>("fields") {
            for pair in fields_table.pairs::<usize, LuaTable>().flatten() {
